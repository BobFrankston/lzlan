...
const lants_1 = require("../src/lants");
    ...
    try {
        if (fs.existsSync(devicesPath)) {
            ...
            for (var xn in xdevices) {
                const d = xdevices[xn];
                let nd = await lants_1.Lifx.createDevice(d);
...
async function getMoreDevices() {
    ...
    try {
        ...
        const moreDevices = await lants_1.Lifx.discover();
        ...
        devices.sort((a, b) => {
            ...
            else if (!b.deviceInfo || !b.deviceInfo.label)
                return 1;
...
async function createDevice(mac, ip) {
    ...
    try {
        ...
        if (lds.length == 1)
            return lds[0];
        ldev = await lants_1.Lifx.createDevice({ mac: mac, ip: ip });
